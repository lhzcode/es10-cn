...
| GetThisBinding() | 返回此[Environment Record](http://zhoushengfe.com/es6/es6-ch.html#sec-environment-records)`this绑定。 如果this绑定尚未初始化，则抛出 ReferenceError 。                            ` |
| GetSuperBase()   | 返回在[Environment Record](http://zhoushengfe.com/es6/es6-ch.html#sec-environment-records)中作为` super `属性访问绑定的基础的对象。该对象源于此[Environment Record](http://zhoushengfe.com/es6/es6-ch.html#sec-environment-records)的[[ HomeObject]]字段。 值 undefined 表示`super`属性访问将产生运行时错误. |
